...
def train_model(model, dataset, params, ckpt, ckpt_manager, out_file):
  ...
  def train_step(enc_inp, enc_extended_inp, dec_inp, dec_tar, batch_oov_len):
    ...
    with tf.GradientTape() as tape:
      ...
      enc_hidden,enc_state, enc_output = model.call_encoder(enc_inp)
      ##Changes
      #print('Got the encout put and enc hidden')
      predictions, _ = model(enc_output, enc_hidden,enc_state, enc_inp, enc_extended_inp, dec_inp, batch_oov_len)
  ...
  try:
    ...
    for batch in dataset:
      ...
      f.write('Step {}, time {:.4f}, Loss {:.4f}\n'.format(int(ckpt.step),
                                                       time.time()-t0,
                                                       loss.numpy()))
      step_store.append(int(ckpt.step))
      time_store.append(time.time()-t0)
      loss_store.append(loss.numpy())
      if int(ckpt.step) == params["max_steps"]:
        ckpt_manager.save(checkpoint_number=int(ckpt.step))
        print("Saved checkpoint for step {}".format(int(ckpt.step)))
        with open('text0.csv', 'w') as f:
          writer = csv.writer(f, delimiter='\t')
          writer.writerows(zip(time_store,loss_store))
        f.close()
        break
