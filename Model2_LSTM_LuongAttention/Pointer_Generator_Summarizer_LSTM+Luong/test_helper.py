...
def beam_decode(model, batch, vocab, params):
  ...
  def decode_onestep(batch, enc_outputs, dec_state,cell_state, dec_input):
    """
    ...
    final_dists, dec_hidden, context_vector, attentions, p_gens = model(enc_outputs, dec_state,cell_state,batch[0]["enc_input"], batch[0]["extended_enc_input"], dec_input, batch[0]["max_oov_len"])
  ...
  state,cell_state, enc_outputs = model.call_encoder(batch[0]["enc_input"])
  ...
  while steps < params['max_dec_steps'] and len(results) < params['beam_size'] : 
    ...
    returns = decode_onestep( batch, enc_outputs, tf.stack(states, axis=0),cell_state, tf.expand_dims(latest_tokens, axis=1))
