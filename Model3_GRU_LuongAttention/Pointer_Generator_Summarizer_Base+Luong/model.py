...
from layers import Encoder, LuongAttention, Decoder, Pointer
...
class PGN(tf.keras.Model):
  ...
  def call(self, enc_output, dec_hidden, enc_inp, enc_extended_inp,  dec_inp, batch_oov_len):
    ...
    for t in range(dec_inp.shape[1]):
      dec_x, pred, dec_hidden,context_vector, attn = self.decoder(tf.expand_dims(dec_inp[:, t],1), dec_hidden, enc_output, context_vector)
